...
import { isInstalled } from '../../src/api.ts'
...
describe('isInstalled', () => {
  ...
  test('returns true when only an installCheckPaths entry exists (systemPath absent)', async () => {
    // Regression coverage: OpenCode's global `opencode.json` is
    // USER-CREATED. A fresh install has no systemPath file or parent
    // yet, but the catalog's installCheckPaths point at OpenCode's
    // data dirs. The probe must recognize this state as "installed",
    // or downstream UIs hide the row post-Disconnect.
