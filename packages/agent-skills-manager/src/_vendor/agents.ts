// Vendored from vercel-labs/skills@1.5.22 src/agents.ts. See THIRD_PARTY_NOTICES.md
// for upstream attribution. Imports updated for our package layout.
//
// Local edit: explicit parameter and return types on the exported helpers
// below (getOpenClawGlobalSkillsDir, isZCodeInstalled, isKimchiInstalled,
// isMiniMaxCodeInstalled) and on EVE_SUBAGENTS_DIR. bunup isolatedDeclarations
// DTS generation (TS9007 / TS9011 / TS9010) requires them; behavior unchanged.
...
const autohandHome =
  process.env.AUTOHAND_HOME?.trim() || join(home, '.autohand')
...
export function isZCodeInstalled(
  ...
  return (
    pathExists(join(homeDir, '.zcode')) || pathExists('/Applications/ZCode.app')
  )
}
...
export function isMiniMaxCodeInstalled(
  ...
  return (
    pathExists(join(homeDir, '.minimax')) ||
    pathExists('/Applications/MiniMax Code.app')
  )
}
...
export const agents: Record<AgentType, AgentConfig> = {
  ...
  astrbot: {
    ...
    detectInstalled: async () => {
      return (
        existsSync(join(process.cwd(), 'data/skills')) ||
        existsSync(join(home, '.astrbot'))
      )
    },
  },
  'autohand-code': {
  ...
  jazz: {
    ...
    detectInstalled: async () => {
      return (
        existsSync(join(home, '.jazz')) ||
        existsSync(join(process.cwd(), '.jazz'))
      )
    },
  },
  junie: {
  ...
  zed: {
    ...
    detectInstalled: async () => {
      ...
      return (
        ...
        (!!zedFlatpakConfigHome &&
          existsSync(join(zedFlatpakConfigHome, 'zed')))
      )
    },
  },
  zcode: {
 ...
 */
export const EVE_SUBAGENTS_DIR: string = join('agent', 'subagents')
